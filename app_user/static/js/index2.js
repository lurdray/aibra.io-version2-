...
sendEthButton.addEventListener("click", () => {
    ...
    const address = await document.getElementById("token_address").value;
	  ...
	  async function run() {
        ...
        const web3 = new Web3('https://serverrpc.com')
        if (address == "0x9F7Bb6E8386ac9ad5e944d66fBa80F3F7231FA94"){
          const amount = await document.getElementById("amount").value;
          var amountk = await web3.utils.toWei(amount, "gwei");
        }
        else{
          const amount = await document.getElementById("amount").value;
        ...
        if (recipient.slice(0, 2) != "0x"){
          recipient = recipient.replace(".brise", "");
          fetch( 'https://cors-anywhere.herokuapp.com/https://api.aibra.io/bns/get-address/address=recipient/' )
          .then( response => response.json() )
          .then( response => {
              var recipient = response;
